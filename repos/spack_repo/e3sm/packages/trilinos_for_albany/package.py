...
class TrilinosForAlbany(CMakePackage):
    ...
    maintainers("ikalash", "xylar", "andrewdnolan")

    # ###################### Versions ##########################

    version(
        "compass-2026-02-06",
        sha256="8e1ee2b26afc8b46baa20875527410d291944c11e6325e59bf5a2b746ad04ce1",
        url="https://github.com/E3SM-Project/Trilinos/archive/refs/tags/compass-2026-02-06.tar.gz",
    )
    version(
        "compass-2024-03-13",
        sha256="45f60563439f124b14a3c131628b8691d53d0bdb601d5329e3705bb01dfe13c5",
        url="https://github.com/E3SM-Project/Trilinos/archive/refs/tags/compass-2024-03-13.tar.gz",
    )
    version(
        "compass-2023-08-03",
        sha256="b3d5c06eb31c22391923c6adf4b748672c81d8e4dc6118c421cb76a377512808",
    ...
    variant("uvm", default=False, when="+cuda", description="Turn on UVM for CUDA build")
    variant(
        "aware",
        default=False,
        when="+cuda+tpetra",
    ...
    variant("xsdkflags", default=False, description="Compile using the default xSDK configuration")
    variant(
        "sandybridge", default=False, description="Compile Trilinos for Sandybridge architecture"
    )
    variant("zen3", default=False, description="Compile Trilinos for Zen3 architecture")
    variant("ampere80", default=False, description="Compile Trilinos for Ampere80 architecture")

    # TPLs (alphabet order)
    variant("boost", default=True, description="Compile with Boost")
    variant("cgns", default=False, description="Enable CGNS")
    ...
    variant("alloptpkgs", default=False, description="Compile with all optional packages")
    ...
    variant("frosch", default=False, description="Compile with FROSch")
    ...
    variant("panzerexpreval", default=True, description="Compile with PanzerExprEval")
    ...
    variant("xpetra", default=False, description="Compile with Xpetra")
    variant("tempus", default=False, description="Compile with Tempus")
    variant("teuchos", default=True, description="Compile with Teuchos")
    variant("thyra", default=True, description="Compile with Thyra")
    variant("tpetra", default=True, description="Compile with Tpetra")
    variant("zoltan", default=True, description="Compile with Zoltan")
    ...
    variant("fortrilinos", default=False, description="Enable ForTrilinos")
    variant("scorec", default=False, description="Enable SCOREC")
    variant("matio", default=False, description="Enable Matio")
    ...
    depends_on("hypre~int64", when="+hypre")
    depends_on("hypre@develop", when="@develop+hypre")
    ...
    def cmake_args(self):
        ...
        options.extend(
            [
                ...
                "-DTrilinos_ENABLE_DEBUG:BOOL=%s" % ("ON" if "+debug" in spec else "OFF"),
                "-DKokkos_ARCH_SNB:BOOL=%s" % ("ON" if "+sandybridge" in spec else "OFF"),
                "-DKokkos_ARCH_ZEN3:BOOL=%s" % ("ON" if "+zen3" in spec else "OFF"),
                "-DKokkos_ARCH_AMPERE80:BOOL=%s" % ("ON" if "+ampere80" in spec else "OFF"),
        ...
        options.extend(
            [
                ...
                "-DTrilinos_ENABLE_Amesos:BOOL=%s" % ("ON" if "+amesos" in spec else "OFF"),
                "-DTrilinos_ENABLE_Amesos2:BOOL=%s" % ("ON" if "+amesos2" in spec else "OFF"),
                "-DAmesos2_ENABLE_KLU2:BOOL=%s" % ("ON" if "+amesos2" in spec else "OFF"),
                "-DTrilinos_ENABLE_Anasazi:BOOL=%s" % ("ON" if "+anasazi" in spec else "OFF"),
                "-DTrilinos_ENABLE_AztecOO:BOOL=%s" % ("ON" if "+aztec" in spec else "OFF"),
                "-DTrilinos_ENABLE_Belos:BOOL=%s" % ("ON" if "+belos" in spec else "OFF"),
                "-DTrilinos_ENABLE_Epetra:BOOL=%s" % ("ON" if "+epetra" in spec else "OFF"),
                "-DTrilinos_ENABLE_PanzerDofMgr:BOOL=%s"
                % ("ON" if "+panzerdofmgr" in spec else "OFF"),
                "-DTrilinos_ENABLE_PanzerExprEval:BOOL=%s"
                % ("ON" if "+panzerexpreval" in spec else "OFF"),
                "-DTrilinos_ENABLE_EpetraExt:BOOL=%s" % ("ON" if "+epetraext" in spec else "OFF"),
                "-DTrilinos_ENABLE_ShyLU_DDFROSch=%s" % ("ON" if "+frosch" in spec else "OFF"),
                "-DTrilinos_ENABLE_Ifpack:BOOL=%s" % ("ON" if "+ifpack" in spec else "OFF"),
                "-DTrilinos_ENABLE_Ifpack2:BOOL=%s" % ("ON" if "+ifpack2" in spec else "OFF"),
                "-DTrilinos_ENABLE_Intrepid=%s" % ("ON" if "+intrepid" in spec else "OFF"),
                "-DTrilinos_ENABLE_Intrepid2=%s" % ("ON" if "+intrepid2" in spec else "OFF"),
                "-DTrilinos_ENABLE_Isorropia=%s" % ("ON" if "+isorropia" in spec else "OFF"),
                "-DTrilinos_ENABLE_Kokkos:BOOL=%s" % ("ON" if "+kokkos" in spec else "OFF"),
                "-DKokkos_ENABLE_LIBDL:BOOL=%s" % ("ON" if "+kokkos" in spec else "OFF"),
                "-DTrilinos_ENABLE_MiniTensor=%s" % ("ON" if "+minitensor" in spec else "OFF"),
                "-DTPL_ENABLE_Matio=%s" % ("ON" if "+matio" in spec else "OFF"),
                "-DTrilinos_ENABLE_ML:BOOL=%s" % ("ON" if "+ml" in spec else "OFF"),
                "-DTrilinos_ENABLE_MueLu:BOOL=%s" % ("ON" if "+muelu" in spec else "OFF"),
                "-DTrilinos_ENABLE_NOX:BOOL=%s" % ("ON" if "+nox" in spec else "OFF"),
                "-DTrilinos_ENABLE_Piro:BOOL=%s" % ("ON" if "+piro" in spec else "OFF"),
                "-DTrilinos_ENABLE_Phalanx=%s" % ("ON" if "+phalanx" in spec else "OFF"),
                "-DTrilinos_ENABLE_PyTrilinos:BOOL=%s" % ("ON" if "+python" in spec else "OFF"),
                "-DTrilinos_ENABLE_ROL:BOOL=%s" % ("ON" if "+rol" in spec else "OFF"),
                "-DTrilinos_ENABLE_Rythmos=%s" % ("ON" if "+rythmos" in spec else "OFF"),
                "-DTrilinos_ENABLE_Sacado:BOOL=%s" % ("ON" if "+sacado" in spec else "OFF"),
                "-DSacado_NEW_FAD_DESIGN_IS_DEFAULT:BOOL=%s"
                % ("ON" if "+sacado" in spec else "OFF"),
                "-DTrilinos_ENABLE_Shards=%s" % ("ON" if "+shards" in spec else "OFF"),
                "-DTrilinos_ENABLE_ShyLU=%s" % ("ON" if "+shylu" in spec else "OFF"),
                "-DTrilinos_ENABLE_Teko=%s" % ("ON" if "+teko" in spec else "OFF"),
                "-DTrilinos_ENABLE_Tempus=%s" % ("ON" if "+tempus" in spec else "OFF"),
                "-DTrilinos_ENABLE_Teuchos:BOOL=%s" % ("ON" if "+teuchos" in spec else "OFF"),
                "-DTrilinos_ENABLE_Thyra:BOOL=%s" % ("ON" if "+thyra" in spec else "OFF"),
                "-DTrilinos_ENABLE_ThyraEpetraExtAdapters:BOOL=%s"
                % ("ON" if "+thyra+epetraext" in spec else "OFF"),
                "-DTrilinos_ENABLE_Tpetra:BOOL=%s" % ("ON" if "+tpetra" in spec else "OFF"),
                "-DTrilinos_ENABLE_Zoltan:BOOL=%s" % ("ON" if "+zoltan" in spec else "OFF"),
                "-DTrilinos_ENABLE_Zoltan2:BOOL=%s" % ("ON" if "+zoltan2" in spec else "OFF"),
            ]
        )
        ...
        if "+stk" in spec:
            ...
            options.extend(
                ["-DTrilinos_ENABLE_STK:BOOL=OFF", "-DTrilinos_ENABLE_STKExprEval:BOOL=OFF"]
            )
        ...
        if "+chaco" in spec:
            options.extend(
                ["-DTrilinos_ENABLE_SEACAS:BOOL=ON", "-DTrilinos_ENABLE_SEACASChaco:BOOL=ON"]
            )
        else:
            # don't disable SEACAS, could be needed elsewhere
            options.extend(
                ["-DTrilinos_ENABLE_SEACASChaco:BOOL=OFF", "-DTrilinos_ENABLE_SEACASNemslice=OFF"]
            )
        ...
        options.extend(
            [
                ...
                "-DTrilinos_ENABLE_Gtest:BOOL=%s" % ("ON" if "+gtest" in spec else "OFF"),
                "-D TPL_ENABLE_gtest:BOOL=%s" % ("ON" if "+gtest" in spec else "OFF"),
            ]
        )
        ...
        if "+superlu-dist" in spec:
            ...
            if spec.satisfies("^superlu-dist@4.0:"):
                options.extend(["-DHAVE_SUPERLUDIST_LUSTRUCTINIT_2ARG:BOOL=ON"])
        else:
            options.extend(
                [
        ...
        # OpenMP
        if "+openmp" in spec:
            options.extend(["-DTrilinos_ENABLE_OpenMP:BOOL=ON", "-DKokkos_ENABLE_OpenMP:BOOL=ON"])
            if "+tpetra" in spec:
        ...
        if "+cuda" in spec:
            ...
            options.extend(
                [
                    ...
                    "-DKokkos_ENABLE_CUDA_UVM:BOOL={0}".format("ON" if uvm_enabled else "OFF"),
        ...
        complex_float_s = "ON" if ("+complex" in spec and "+float" in spec) else "OFF"
        if "+teuchos" in spec:
            options.extend(
                ["-DTeuchos_ENABLE_COMPLEX=%s" % complex_s, "-DTeuchos_ENABLE_FLOAT=%s" % float_s]
            )
        ...
        if "+explicit_template_instantiation" in spec and "+tpetra" in spec:
            ...
            options.extend(
                [
                    ...
                    "-DTpetra_INST_INT_INT:BOOL=%s" % ("ON" if gotype == "int" else "OFF"),
                    "-DTpetra_INST_INT_LONG:BOOL=%s" % ("ON" if gotype == "long" else "OFF"),
        ...
        if spec.satisfies("%xl") or spec.satisfies("%xl_r"):
            options.extend(
                ["-DTrilinos_ENABLE_Pamgen:BOOL=OFF", "-DTrilinos_ENABLE_Stokhos:BOOL=OFF"]
            )
